...
export default function Footer() {
  ...
  // Safely determine the role (defaults to 'guest' if not logged in)
  const role = isAuthenticated && user ? user.role.toLowerCase() : "guest";

  // 🌟 DYNAMIC CONFIGURATION: Changes content based on user role


  // Get the specific content for the current role
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-12 md:gap-8 mb-16">
          ...
          <div className="md:col-span-4 lg:col-span-5">
            ...
            <p className="text-gray-400 leading-relaxed text-sm max-w-sm mb-8">
              The most elegant way to find, book, and manage premium local services. We bring verified professionals directly to your door.
            </p>
          ...
          <div className="md:col-span-8 lg:col-span-7 grid grid-cols-2 sm:grid-cols-3 gap-8">
            ...
            {/* Column 1 */}
            <div>
              <h4 className="font-bold text-gray-100 tracking-wider mb-6 text-sm uppercase">{content.col1.title}</h4>
              <ul className="space-y-4 text-sm text-gray-400">
                {content.col1.links.map((link, i) => (
                  <li key={i}>
                    <Link to={link.path} className="group flex items-center hover:text-white transition-colors duration-300 w-fit">
                      <span className="group-hover:translate-x-1.5 transition-transform duration-300 ease-out">{link.name}</span>
                    </Link>
                  </li>
                ))}
              </ul>
            ...
            {/* Column 2 */}
            <div>
              <h4 className="font-bold text-gray-100 tracking-wider mb-6 text-sm uppercase">{content.col2.title}</h4>
              <ul className="space-y-4 text-sm text-gray-400">
            ...
            <div className="col-span-2 sm:col-span-1 mt-4 sm:mt-0">
              <h4 className="font-bold text-gray-100 tracking-wider mb-6 text-sm uppercase">{content.cta.title}</h4>
              <Link 
                to={content.cta.path} 
              ...
              >
                {content.cta.btnText} <ArrowRight size={16} className="text-gray-900 group-hover:translate-x-1 transition-transform" />
              </Link>
            </div>
          </div>
        </div>
